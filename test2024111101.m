...
function varargout = test2024111101(varargin)
...
gui_Singleton = 1;
%gui_State 是一个结构体，用于存储GUI的各种状态信息。这些状态信息包括GUI的名称、是否为单例、打开函数、输出函数、布局函数以及回调函数的引用等。
gui_State = struct('gui_Name',       mfilename, ...%gui_Name 字段存储了GUI的文件名（通过mfilename函数获取），这有助于识别正在运行的GUI。
                   'gui_Singleton',  gui_Singleton, ...%gui_Singleton = 1; 表示这个GUI被设计为单例（Singleton）。单例GUI意味着在同一时间只能有一个该GUI的实例在运行。如果尝试打开另一个实例，MATLAB会激活已经存在的实例而不是创建一个新的。
                   'gui_OpeningFcn', @GINavCfg_OpeningFcn, ...%gui_OpeningFcn 字段存储了打开GUI时要调用的函数的引用。在这个例子中，它引用了GINavCfg_OpeningFcn函数，这个函数通常在GUI打开时执行初始化操作。
                   'gui_OutputFcn',  @GINavCfg_OutputFcn, ...%gui_OutputFcn 字段存储了输出函数的引用，这个函数用于处理GUI的输出（如命令窗口中的消息）。在这个例子中，它引用了GINavCfg_OutputFcn函数。
                   'gui_LayoutFcn',  [] , ...%gui_LayoutFcn 字段通常用于存储布局函数的引用，但在这个例子中它被设置为空（[]），表示没有为GUI指定布局函数。
                   'gui_Callback',   []);%gui_Callback 字段也设置为空（[]），但在更复杂的GUI中，它可能用于存储回调函数的列表或引用。

if nargin && ischar(varargin{1})
    gui_State.gui_Callback = str2func(varargin{1});
end
% - `nargin`是一个内置函数，它返回当前函数接收到的输入参数的数量。
% - `varargin`是一个特殊变量，用于接收函数的可变输入参数。它是一个cell数组，其中每个元素都是传递给函数的参数。
% - `ischar(varargin{1})`检查`varargin`中的第一个元素（`varargin{1}`）是否是一个字符数组（即字符串）。
% - `&&`是逻辑与操作符，它要求两边的表达式都为真（非零、非空）时，整个表达式才为真。

if nargout%在MATLAB中，`nargout` 是一个内置函数，用于返回当前函数被调用时指定的输出参数的数量。这个函数在函数体内部使用，可以帮助函数根据输出参数的数量来决定执行不同的操作或返回不同的数据。
    [varargout{1:nargout}] = gui_mainfcn(gui_State, varargin{:});
else
    gui_mainfcn(gui_State, varargin{:});
end
% End initialization code - DO NOT EDIT
%这些代码通常位于GUI的初始化脚本中，该脚本在GUI打开时自动执行。
%用户不应直接修改这些由GUIDE或App Designer生成的代码，除非他们完全理解这些代码的作用，并且知道修改可能带来的后果。
%如果需要自定义GUI的行为，通常建议通过添加或修改用户定义的回调函数来实现，而不是直接修改这些自动生成的代码。
